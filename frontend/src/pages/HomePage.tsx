...
import { api, type ApiService, type ApiReview, type ApiStats } from '../lib/api'
...
export function HomePage() {
  ...
  const statCards = stats ? [
    { value: stats.totalRevenue > 0 ? `₴${Math.round(stats.totalRevenue / 1000)}k+` : '₴100k+', label: 'Виконано робіт' },
    { value: stats.avgRating > 0 ? `★ ${stats.avgRating.toFixed(1)}` : '★ 4.8', label: 'Середня оцінка' },
    { value: `${stats.clientCount > 0 ? stats.clientCount : services.length * 3}+`, label: 'Постійних клієнтів' },
    { value: `${stats.serviceCount}`, label: 'Послуг у каталозі' },
  ] : [
    { value: '₴100k+', label: 'Виконано робіт' },
    { value: '★ 4.8',  label: 'Середня оцінка' },
    { value: '20+',    label: 'Постійних клієнтів' },
    { value: '8',      label: 'Послуг у каталозі' },
  ]
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-32 w-full">
          <div className="max-w-2xl">
            ...
            <motion.div {...fadeUp(0.4)} className="flex flex-wrap gap-4">
              <button onClick={() => navigate('/booking')} className="px-8 py-3.5 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold shadow-2xl shadow-blue-500/30 hover:shadow-blue-500/50 transition-all duration-300 hover:-translate-y-0.5">
                Записатись зараз
              </button>
              <button onClick={() => navigate('/services')} className="px-8 py-3.5 rounded-2xl bg-zinc-800 border border-zinc-700 text-zinc-200 font-semibold hover:bg-zinc-700 transition-colors">
                Наші послуги →
              </button>
            </motion.div>
          </div>

          <motion.div {...fadeUp(0.5)} className="absolute right-0 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-4">
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 text-zinc-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredServices.map((s, i) => (
              <motion.div key={s.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -4 }} className="bg-zinc-900 border border-zinc-800/60 rounded-2xl overflow-hidden group">
                <div className={`h-36 bg-gradient-to-br ${s.gradient} flex items-center justify-center`}>
